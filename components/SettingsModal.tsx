...
import { X, Settings, MousePointer2, Palette, ShieldAlert, Keyboard, Check, PaintBucket, Timer, MenuSquare, RefreshCw, FolderSync } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onUpdateSettings, onSyncLibrary }) => {
  ...
  const handleSyncTitles = async () => {
      ...
      setIsSyncing(true);
      try {
          const res = await fetch('http://localhost:8000/sync_manga_names', { method: 'POST' });
          if (res.ok) {
              const data = await res.json();
              alert(`同步完成：更新了 ${data.count} 部漫画的标题。`);
              if (onSyncLibrary) onSyncLibrary();
          } else {
    ...
    >
      ...
      >
        ...
        <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 ml-1 flex items-center gap-2">
                <FolderSync className="w-3 h-3" /> 库管理
            </h4>
